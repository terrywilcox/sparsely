...
-export([parse_fun/1, one_digit/1, parse_skip/1, parse_ignore/1]).
...
parse_skip(_Config) ->
    ...
    Result2 = Parser("3x69"),
    ?assertEqual({ok, "x69", <<>>}, Result2).

parse_ignore(_Config) ->
    Digit = ?DIGIT,
    Lower = ?LOWERCASE,
    Ignore = sparsely:ignore(Digit),
    Parser = sparsely:chain([Ignore, Lower, Digit, Digit]),

    assertError(Parser("x69")),
...
parse_optional_with_long_default(_Config) ->
    Digit = ?DIGIT,
    Lower = ?LOWERCASE,
    Optional = sparsely:optional(Digit, "fred"),
    Parser = sparsely:chain([Optional, Lower, Digit, Digit]),

    Result = Parser("x69"),
    ?assertEqual({ok, ["fred", $x, $6, $9], <<>>}, Result).
...
parse_fun(_Config) ->
    Parser = sparsely:parse_fun(sparsely_SUITE, one_digit),
    Result = Parser("3456"),
